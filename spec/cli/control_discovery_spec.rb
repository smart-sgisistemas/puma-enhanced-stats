...
RSpec.describe Puma::Enhanced::Stats::CLI::ControlDiscovery do
  ...
  it "applies CLI overrides after config and state file resolution" do
    state_path = write_state <<~STATE
      ---
      control_url: tcp://127.0.0.1:9494
      control_auth_token: from-state
    ...
    entry = nil
    Dir.chdir tmpdir do
      entry = described_class.resolve overrides: {
        control_url: "http://127.0.0.1:7777",
        token: "cli-token",
        state_path: state_path
      }
    end

    expect(entry.control_url).to eq("http://127.0.0.1:7777")
    expect(entry.token).to eq "cli-token"
    expect(entry.master_pid).to eq 4242
  end

  it "loads an explicit config file from -F override" do
    config_path = write_config <<~RUBY
      activate_control_app "http://127.0.0.1:8888", auth_token: "from-flag"
    RUBY

    entry = described_class.resolve overrides: { config_path: config_path }

    expect(entry.control_url).to eq("http://127.0.0.1:8888")
    expect(entry.token).to eq "from-flag"
  end
end
