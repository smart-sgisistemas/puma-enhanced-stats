...
RSpec.describe Puma::Enhanced::Stats::CLI::Terminal do
  ...
  it "clears the screen when attached to a TTY" do
    ...
    expect { described_class.clear }.to output("\e[0m\e[H\e[2J").to_stdout
  end
  ...
  it "enters the alternate screen on a TTY" do
    described_class.tty_override = true
    described_class.alternate_active = false

    expect { described_class.enter_alternate_screen! }
      .to output("#{described_class::ALT_ENTER}#{described_class::MOUSE_ENABLE}").to_stdout
    expect(described_class.alternate_active).to be true
  end

  it "skips alternate screen when not a TTY" do
    described_class.tty_override = false
    described_class.alternate_active = false

    expect { described_class.enter_alternate_screen! }.not_to output(/\e/).to_stdout
    expect(described_class.alternate_active).to be false
  end
end
