...
RSpec.describe Puma::Enhanced::Stats::CLI::Fetcher do
  ...
  it "accepts a pre-resolved connection entry" do
    entry = Puma::Enhanced::Stats::CLI::ControlDiscovery::Entry.new(
  ...
  it "passes overrides to ControlDiscovery when no resolved entry is given" do
    expect(Puma::Enhanced::Stats::CLI::ControlDiscovery).to receive(:resolve)
      .with(overrides: { control_url: "http://127.0.0.1:1" })
      .and_return(
        Puma::Enhanced::Stats::CLI::ControlDiscovery::Entry.new(
    ...
    described_class.new(overrides: { control_url: "http://127.0.0.1:1" }).fetch
  end
end
